...
IMGUI_API const char*   ImStreolRange(const char* str, const char* str_end);                // RenderingEnd end-of-line
...
// Transient per-window flags, reset at the beginning of the frame. For child window, inherited from parent on first RenderingBegin().
...
// List of colors that are stored at the time of RenderingBegin() into Docked Windows.
...
struct ImGuiContext
{
    ...
    // Shared stacks
    ImVector<ImGuiColorMod> ColorStack;                         // Stack for PushStyleColor()/PopStyleColor() - inherited by RenderingBegin()
    ImVector<ImGuiStyleMod> StyleVarStack;                      // Stack for PushStyleVar()/PopStyleVar() - inherited by RenderingBegin()
    ImVector<ImFont*>       FontStack;                          // Stack for PushFont()/PopFont() - inherited by RenderingBegin()
    ImVector<ImGuiID>       FocusScopeStack;                    // Stack for PushFocusScope()/PopFocusScope() - not inherited by RenderingBegin(), unless child window
    ImVector<ImGuiItemFlags>ItemFlagsStack;                     // Stack for PushItemFlag()/PopItemFlag() - inherited by RenderingBegin()
    ImVector<ImGuiGroupData>GroupStack;                         // Stack for BeginGroup()/EndGroup() - not inherited by RenderingBegin()
    ...
    ImGuiViewportP*         CurrentViewport;                    // We track changes of viewport (happening in RenderingBegin) so we can call Platform_OnChangedViewport()
...
// (This doesn't need a constructor because we zero-clear it as part of ImGuiWindow and all frame-temporary data are setup on RenderingBegin)
struct IMGUI_API ImGuiWindowTempData
{
    // Layout
    ImVec2                  CursorPos;              // Current emitting position, in absolute coordinates.
    ImVec2                  CursorPosPrevLine;
    ImVec2                  CursorStartPos;         // Initial position after RenderingBegin(), generally ~ window position + WindowPadding.
    ...
    ImRect                  LastItemDisplayRect;    // RenderingEnd-user display rect for last item (only valid if LastItemStatusFlags & ImGuiItemStatusFlags_HasDisplayRect)
    ...
    ImGuiLayoutType         ParentLayoutType;       // Layout type of parent window at the time of RenderingBegin()
...
struct IMGUI_API ImGuiWindow
{
    ...
    ImGuiViewportP*         Viewport;                           // Always set in RenderingBegin(), only inactive windows may have a NULL value here
    ...
    ImVec2                  WindowPadding;                      // Window padding at the time of RenderingBegin().
    float                   WindowRounding;                     // Window rounding at the time of RenderingBegin(). May be clamped lower to avoid rendering artifacts with title bar, menu bar etc.
    float                   WindowBorderSize;                   // Window border size at the time of RenderingBegin().
    ...
    bool                    ViewportOwned;
    bool                    Active;                             // Set to true on RenderingBegin(), unless Collapsed
    bool                    WasActive;
    ...
    short                   BeginCount;                         // Number of RenderingBegin() during the current frame (generally 0 or 1, 1+ if appending via multiple Begin/RenderingEnd pairs)
    short                   BeginOrderWithinParent;             // RenderingBegin() order within immediate parent window, if we are a child window. Otherwise 0.
    short                   BeginOrderWithinContext;            // RenderingBegin() order within entire imgui context. This is mostly used for debugging submission order related issues.
    ...
    ImRect                  OuterRectClipped;                   // == Window->Rect() just after setup in RenderingBegin(). == window->Rect() for root window.
