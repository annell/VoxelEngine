...
bool ImGui::ScrollbarEx(const ImRect& bb_frame, ImGuiID id, ImGuiAxis axis, float* p_scroll_v, float size_avail_v, float size_contents_v, ImDrawFlags flags)
{
    ...
    // Handle input right away. None of the code of RenderingBegin() is relying on scrolling position before calling Scrollbar().
    bool held = false;
    bool hovered = false;
    ...
    if (held && allow_interaction && grab_h_norm < 1.0f)
    {
        ...
        // It is ok to modify Scroll here because we are being called in RenderingBegin() after the calculation of ContentSize and before setting up our starting position
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    if (g.ActiveId == id && !g.ActiveIdIsJustActivated && !clear_active_id)
    {
        ...
        const bool is_startend_key_down = is_osx && io.KeySuper && !io.KeyCtrl && !io.KeyAlt;  // OS X style: Line/Text Start and End using Cmd+Arrows instead of Home/RenderingEnd
...
bool ImGui::BeginMenu(const char* label, bool enabled)
{
    ...
    // If a menu with same the ID was already submitted, we will append to it, matching the behavior of RenderingBegin().
    ...
    // The reference position stored in popup_pos will be used by RenderingBegin() to find a suitable position for the child menu,
    ...
    if (window->DC.LayoutType == ImGuiLayoutType_Horizontal)
    {
        ...
        // For ChildMenu, the popup position will be overwritten by the call to FindBestWindowPosForPopup() in RenderingBegin()
